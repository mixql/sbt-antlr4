...
object Antlr4Plugin extends AutoPlugin {
  object autoImport {
    ...
    val antlr4Generate =
      taskKey[Seq[File]]("Generate classes from antlr4 grammars")
    val antlr4RuntimeDependency =
      settingKey[ModuleID]("Library dependency for antlr4 runtime")
    val antlr4Dependency =
      settingKey[ModuleID]("Build dependency required for parsing grammars")
    val antlr4PackageName =
      settingKey[Option[String]]("Name of the package for generated classes")
  ...
  private val antlr4BuildDependency = settingKey[ModuleID](
    "Build dependency required for parsing grammars, scoped to plugin"
  )
  ...
  def antlr4GeneratorTask: Def.Initialize[Task[Seq[File]]] = Def.task {
    ...
    val cachedCompile = FileFunction.cached(
      streams.value.cacheDirectory / "antlr4",
  ...
  def runAntlr(
    ...
    val listenerArgs =
      if (listenerOpt) Seq("-listener") else Seq("-no-listener")
    val visitorArgs = if (visitorOpt) Seq("-visitor") else Seq("-no-visitor")
    val warningAsErrorArgs =
      if (warningsAsErrorOpt) Seq("-Werror") else Seq.empty
    val sourceArgs = srcFiles.map {
      _.toString
    }
    val args =
      baseArgs ++ packageArgs ++ listenerArgs ++ visitorArgs ++ warningAsErrorArgs ++ sourceArgs
